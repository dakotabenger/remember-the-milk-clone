...
document.addEventListener('DOMContentLoaded', event => {
    ...
    form.addEventListener('submit',async (e) => {
         ...
         const res = await fetch('/api/task', {
             ...
             body: JSON.stringify({  //probably need to change this part
               name, description, priority, startDate, endDate, completed
             })
         })
         const json = await res.json()
         form.reset()
         const showTasks = await receiveTaskFromServer(json)
 ...
 const receiveTaskFromServer = (data) => {
        ...
        newTd.setAttribute("id",`${data.id}`)
        newTd.innerHTML = `${data.name}`
